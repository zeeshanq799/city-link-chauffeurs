...
<div x-data="{ show: true }" 
     x-show="show"
     x-transition
     {{ $attributes->merge(['class' => 'flex items-start p-4 rounded-lg border ' . $config['bg'] . ' ' . $config['border'] . ' ' . $config['text']]) }}>
    ...
    @if($dismissible)
        <button @click="show = false" class="ml-4 flex-shrink-0 hover:opacity-75 transition-opacity">
